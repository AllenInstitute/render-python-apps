...
class CreateSectionPolygons(RenderModule):
    def __init__(self,schema_type=None,*args,**kwargs):
        if schema_type is None:
            schema_type = CreateSectionPolygonsParameters
        super(CreateSectionPolygons,self).__init__(schema_type=schema_type,*args,**kwargs)
    def run(self):
        self.logger.error('WARNING NEEDS TO BE TESTED, TALK TO FORREST IF BROKEN OR WORKS')
        stack = self.args['stack']
        sectionImageDir = self.args['sectionImageDir']
        jsonDir = self.args['polygon_dir']

        #jsonobj,alllines,img=find_section_boundaries(r,stack,self.args['sectionImageDir'],66,plot=True,isHorizontal=True)

        mypartial = partial(find_section_boundaries,self.render,stack,self.args['sectionImageDir'],isHorizontal=self.args['isHorizontal'])
