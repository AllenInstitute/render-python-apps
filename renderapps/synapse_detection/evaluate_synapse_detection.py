...
class EvaluateSynapseDetectionParameters(ArgSchema):
    ...
    EM_inclass_conditions=argschema.fields.NumpyArray(required=False,default=[1],
...
class EvaluateSynapseDetection(ArgSchemaParser):
    ...
    def run(self):
        ...
        in_class_rows = np.isin(df[self.args['EM_inclass_column']],self.args['EM_inclass_conditions'])
        if self.args['EM_inclass_invert']:
            in_class_rows = ~in_class_rows
        good_rows = good_rows & in_class_rows      
