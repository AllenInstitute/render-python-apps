...
def define_roi(roi): #rectangle with a center(x0,y0), width (dx) and height (dy)
    ...
    roi_corners = np.array([[x0 - dx/2, y0 - dy/2],[x0 - dx/2, y0 + dy/2],
                            [x0 + dx/2, y0 + dy/2],[x0 + dx/2, y0 - dy/2]])
    roi_norms = np.array([[x0, y0 + dy/2], [x0 + dx/2, y0], [x0, y0 - dy/2],
                          [x0 - dx/2, y0]])
    return roi_center, roi_corners, roi_norms
...
def roi_to_stage_coord_3d(r, stack, z_range, roi_center, roi_corners, roi_norms):
    zvalues = renderapi.stack.get_z_values_for_stack(stack, render=r)
    roi_center_stage_list = []
    roi_corners_stage_list = []
    ...
    for z in zvalues[z_range[0]:z_range[1] + 1]:
        ...
        roi_corners_stage = np.zeros(roi_corners.shape)
        for i in range(roi_corners.shape[0]):
            x, y = roi_corners[i,:]
            coord_list = renderapi.coordinate.world_to_local_coordinates(
                stack, z, x, y, render=r)
            x_local, y_local = coord_list[-1]['local'][0:2]
            tileId = str(coord_list[-1]['tileId'])
            roi_corners_stage[i,:] = local_to_stage_coord(r, stack, x_local,
                                                          y_local, tileId)
        roi_corners_stage_list.append(roi_corners_stage)
    ...
    return roi_center_stage_list, roi_corners_stage_list, roi_norms_stage_list
...
class CreatePrecisionSites(RenderModule):
    ...
    def run(self):
        ...
        roi_center, roi_corners, roi_norms = define_roi(self.args['roi'])
        roi_center_stage_list, roi_corners_stage_list, roi_norms_stage_list\
        = roi_to_stage_coord_3d(self.render, self.args['stack'], 
                                self.args['zrange'], roi_center, 
                                roi_corners, roi_norms)
